  ...
  return (
    <section id="experience" className="space-y-12">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        ...
        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest bg-slate-100 px-4 py-1 rounded-full">6.5 Years Industry Experience</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-1 space-y-3">
          {experienceData.map((exp)=> (
            <button key={exp.id} onClick={()=>setActive(exp.id)}
              className={`w-full text-left p-6 rounded-2xl transition-all ${active===exp.id?'bg-slate-900 text-white shadow-xl translate-x-2':'hover:bg-slate-100 text-slate-500'}`}>
        ...
        <div className="lg:col-span-3">
          <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-sm min-h-[500px]">
            <h3 className="text-3xl font-extrabold text-slate-900">{current.role}</h3>
            <p className="text-xl text-slate-400 font-semibold mt-1">{current.company} — {current.period}</p>

            <ul className="mt-8 space-y-5">
              {current.highlights.map((line,i)=>(
                <li key={i} className="flex items-start gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-slate-900 mt-2.5"></div>
